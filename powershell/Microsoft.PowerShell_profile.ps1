...
# Función para cambiar de directorio al salir de lf
function lfcd {
...
. "$HOME\Documents\PowerShell\myalias.ps1"
...
if ($UsarOhMyPosh) {
    Invoke-Expression (& oh-my-posh init pwsh --config "$env:USERPROFILE\.oh-my-posh\themes\wopian.omp.json" | Out-String)
}
...
"@
    ...
    # Se definen de forma global (global:) para que existan fuera de esta función
...
# Si el interruptor de arriba está activado, se carga automáticamente.
if ($AutoCargarFZF) {
    Activar-FZF
}
